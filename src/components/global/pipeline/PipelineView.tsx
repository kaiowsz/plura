...
import { LaneDetail, PipelineDetailsWithCardsTagsTickets, TicketAndTags } from "@/lib/types";
...
const PipelineView = ({ lanes, pipelineDetails, pipelineId, subaccountId, updateLanesOrder, updateTicketsOrder }: Props) => {
    ...
    const onDragEnd = (dropResult: DropResult) => {
        ...
        const {  destination, type, source } = dropResult;
        ...
        switch(type) {
            case "lane": {
                const newLanes = [...allLanes].toSpliced(source.index, 1).toSpliced(destination.index, 0, allLanes[source.index]).map((lane, index) => {
                    return {
                        ...lane,
                        order: index
                    }
            ...
            case "ticket": {
                ...
                if(source.droppableId === destination.droppableId) {
                    const newOrderedTickets = [...originLane.Tickets].toSpliced(source.index, 1)
                    ...
                    originLane.Tickets.forEach((ticket, i) => {
                        ticket.order = i;
                    })
                    ...
                    destinationLane.Tickets.forEach((ticket, i) => {
                        ticket.order = i
                    })
    ...
    <DragDropContext onDragEnd={onDragEnd}>
        <div className="bg-white/60 dark:bg-background/60 rounded-xl p-4 use-automation-zoom-in">
            ...
            <Droppable droppableId="lanes" type="lane" direction="horizontal" key="lanes">
                {(provided) => (
                    <div className="flex items-center gap-x-2" {...provided.droppableProps} ref={provided.innerRef}>
                        <div className="flex mt-4">
                            {allLanes.map((lane, i) => (
                                <PipelineLane allTickets={allTickets} setAllTickets={setAllTickets} subaccountId={subaccountId} pipelineId={pipelineId} tickets={lane.Tickets} laneDetails={lane} index={i} key={lane.id} />
                            ))}
