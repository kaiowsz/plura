...
export const upsertSubAccount = async (subAccount: SubAccount) => {
    ...
    if(!agencyOwner) return console.log("Error: Could not create an subaccount.");
...
export const getLanesWithTicketAndTicket = async (pipelineId: string) => {
...
export const updateLanesOrder = async (lanes: Lane[]) => {
    try {
        ...
        console.log(`Error updating lanes: ${error}`)
    }
}
...
export const updateTicketsOrder = async (tickets: Ticket[]) => {
    try {
        ...
        console.log(`Error updating ticket order: ${error}`)    
    }
}
